...
module.exports = async ctx => {
  ...
  if (ctx.data.agent) {
    if (ctx.projectIsPlugin) {
      ctx.spinner.start();
      ctx.spinner.name = 'plugin';
      ctx.spinner.info('正在生成插件Agent文件...');
      await createFiles(ctx, [
        { file: path.resolve(ctx.projectCwd, 'agent.js'), template: 'agent.js.ejs' },
        { file: path.resolve(ctx.projectCwd, 'app', 'message.js'), template: 'app.agent.js.ejs' }
      ]);
